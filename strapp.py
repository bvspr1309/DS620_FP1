...
def calculate_performance():
    ...
    for stock_symbol, stock_info in portfolio.items():
        ...
        portfolio_performance.append({'Symbol': stock_symbol, 'Performance': performance})
...
def main():
    ...
    if not portfolio:
        ...
        performance_data = calculate_performance()
        performance_df = pd.DataFrame(performance_data)
        st.bar_chart(performance_df.set_index('Symbol'))

        st.subheader("Portfolio Risk")
        risk_data = composition_data.merge(performance_df, on='Symbol')
        st.bar_chart(risk_data.set_index('Symbol')['Performance'])
